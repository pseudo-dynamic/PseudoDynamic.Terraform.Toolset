...
{
    public class DynamicValueEncoderTests
    {
        private readonly static DynamicValueDecoder Decoder = new DynamicValueDecoder(new NonRequestableServiceProvider());
        ...
        internal void Encoder_should_encode_then_decode_schemas(object content)
        {
            ...
            var bytes = DynamicValueEncoder.Default.EncodeSchema(block, content);
            var result = Decoder.DecodeSchema(bytes, block, new DynamicValueDecoder.DecodingOptions());
